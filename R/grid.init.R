...
`grid.init` <-
  function(confFile = NULL, localTmpDir = NULL, verbose = TRUE, sshRemoteIp = NULL, sshUsername = NULL, sshRemoteDir = NULL, myProxyHost = NULL, myProxyUsername = NULL, credentialName = NULL, myProxyPwd = NULL, myProxyPort = NULL, service = NULL, sshKey = NULL, debug = FALSE, sharedDir = NULL, remoteRPath = NULL, schedulerIp = NULL, schedulerPort = NULL) {
    ...
    if (!is.null(.grid$localDir) && substring(.grid$localDir, nchar(.grid$localDir)) != "/") {
      .grid$localDir <- paste(.grid$localDir, "/", sep = "")
    }
    if (!is.null(.grid$nfs$dir) && substring(.grid$nfs$dir, nchar(.grid$nfs$dir)) != "/") {
      .grid$nfs$dir <- paste(.grid$nfs$dir, "/", sep = "")
    }
    if (!is.null(.grid$cogDir) && substring(.grid$cogDir, nchar(.grid$cogDir)) != "/") {
      .grid$cogDir <- paste(.grid$cogDir, "/", sep = "")
    }
    if (.grid$service == "condor.ssh" || .grid$service == "remote.ssh") {
      if (substring(.grid$ssh$remotePath, nchar(.grid$ssh$remotePath)) != "/") {
        .grid$ssh$remotePath <- paste(.grid$ssh$remotePath, "/", sep = "")
      }
    }
